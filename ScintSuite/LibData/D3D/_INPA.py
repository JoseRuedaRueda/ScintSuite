"""
Contain routines to interact with the D3D - INPA database
...
import ScintSuite.LibData.D3D.DiagParam as params
...
class INPA_logbook:
    ...
    def getCameraCalibration(self, shot: int,
        ...
        :param  cal_type: Type of calibration we want. Deprecated, not used
        :param  diag_ID: ID of the diagnostic we want

        :return cal: CalParams() object
        """
        if type(self.CameraCalibrationDatabase) == str:
            # We have to select the calibration file with shots smaller that this
            diagName = 'INPA%i'%diag_ID
            folder = os.path.join(self.CameraCalibrationDatabase, diagName)
            shots = []
            for f in os.listdir(folder):
                if f.endswith('.nc'):
                    shots.append(int(f[:6]))
            shots = np.sort(np.array(shots))
            shot1 = shots[np.where(shots < shot)[0][0]]
            fileToLoad = os.path.join(folder, '%i.nc'%shot1)
            calibRaw = xr.load_dataset(fileToLoad)
            cal = CalParams()
            cal.H = calibRaw.H.values
            cal.type = 'Homomorphic'
        else:
    ...
    def getComment(self, shot: int):
        ...
        if self.excelLogbook is not None:
            comment = []
