...
class ApplicationRemapAnalysis:
    ...
    def update_integral(self):
        """Update the integral"""
        print(float(self.r_min.get()), float(self.r_max.get()),
              float(self.xi_min.get()), float(self.xi_max.get()))
        self.integral = self.vid.integrate_remap(
            rlmin=float(self.r_min.get()),
            rlmax=float(self.r_max.get()),
            ximax=float(self.xi_max.get()),
            ximin=float(self.xi_min.get()),
        )
        print(self.integral['integral_in_rl'][:, 54].max())
        print(self.integral['integral_in_xi'][:, 54].max())
    ...
    def plot_reference(self):
        ...
        if self.variable_list.get() == 'xi':
            y = self.integral['integral_in_rl'][:, it].copy()
            x = self.integral['xi']
        else:
            y = self.integral['integral_in_xi'][:, it].copy()
            x = self.integral['rl']
