...
def read_header(filename: str, verbose: bool = False):
    ...
    :return cin_header: dictionary containing header information:
        ...
        - 'Compression':  integer flag to say if the file was compressed or not
            0: Grey images
            1: jpe compressed files
            2: raw uninterpolated color
        - 'FirstMovieImage':  number of the first recorded frame, from trigger
        - 'TotalImageCount': total number of frames taken by the camera
        - 'FirstImageNo': number of the first fame saved, from trigger
        - 'ImageCount': number of saved frames
        - 'OffImageHeader': Offset of the BITMAPINFOHEADER structure in the cin
        - 'OffSetup': Offset of the SETUP structure in the cine file
        - 'OffImageOffsets': Offset in the cine file of an array with the
    ...
    if cin_header['Compression'] == 0:
        ...
        raise Exception('Error reading the header, compression number wrong')
