...
# FILD TOMOGRAPHY
# -----------------------------------------------------------------------------
def FILD_tomography(frame, smap, s_opt: dict, p_opt: dict,
    ...
    # --- Remap the frame
    rep_frame, r, p = ssmapping.remap(smap, frame2, x_min=scint_grid['pmin'],
                                      x_max=scint_grid['pmax'],
                                      delta_x=scint_grid['dp'],
                                      y_min=scint_grid['rmin'],
                                      y_max=scint_grid['rmax'],
                                      delta_y=scint_grid['dp'])
    ...
    if LIMIT_REGION_FCOL:
        ...
        flags_r = (pin_grid['r'] > minr) and (pin_grid['r'] < maxr)
        flags_p = (pin_grid['p'] > minp) and (pin_grid['p'] < maxp)
    ...
    if verbose:
        ...
        print('Gyroradius (cm):', pgrid['rmin'], pgrid['rmax'])
        print('Pitch Angle (degree)', pgrid['pmin'], pgrid['pmax'])
        print('---')
        print('SCINTILLATOR GRID')
        print(scint_grid['nr'], ' x ', scint_grid['np'])
        print('Gyroradius (cm):', scint_grid['rmin'], scint_grid['rmax'])
        print('Pitch Angle (degree)', scint_grid['pmin'], scint_grid['pmax'])
