...
def plotShadedElement(geom: dict, ax=None, surface_params: dict = {},
    ...
    @ToDo: finish the implementation of the 2D case
    """
    # --- Initialise and check settings
    if plot2D:
        raise Exception('Sorry option still not implemented')
    # Plotting options:
    surface_options = {
        'color': 'k',
    ...
    if not triangleFile:
        ...
        for it in range(geom['n']):
            x = np.append(
                geom[key][3*it: (3*it + 3), 0], geom[key][3*it, 0]) * factor
            y = np.append(
                geom[key][3*it: (3*it + 3), 1], geom[key][3*it, 1]) * factor
            z = np.append(
                geom[key][3*it: (3*it + 3), 2], geom[key][3*it, 2]) * factor
            verts = [list(zip(x, y, z))]
            ax.add_collection3d(Poly3DCollection(verts, **surface_options))
