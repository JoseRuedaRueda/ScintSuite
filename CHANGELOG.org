...
- Included method to cut the video in the Video class, to restric to a given region of pixels: Video.cut_frames()
...
- Added fuction to fit a line to a 3d cloud of points
...
- Scan of tomographic reconstruction now gives a dic as output, not single outputs
...
- Included GUI to explore the remaped frames, Vide.GUI_frames_and_remap()
- Improved Video.plot_frame() was upgraded now you can write 'auto' and the function will load and plot the StrikeMap (see its documentation for futher instrucitons)
- LibPlotting.remove_lines() added, it deletes all lines from a plot, usefull to delete the strikemap of one of your plots (used by the new GUIs)
...
- Video.plot_number_saturated_counts() added. If executed without arguments, it plot the pixels counted by default when reading the video. The fuction accept also a threshold, in this case the pixels are count again
...
- The method fildsim.write_namelist() now overwrite by default the existing namelist. You can change this behaviour with the flag =overwrite=
