...
*** Prerequites. Python 3.7 or higher
Needed packages:
...
*** Documentation
- All objects aand methods are documented such that the user can understand what is going on
- Ass everything has docstrings, you can allways write in the python terminal <fname>? and you will get all the description of the <fnam> method or object
- The routines in the Example folder are intended to ilustrate the use of the different tools in the suite. Please, if you want to play with them, make your own copy on 'MyRoutines', modifying the examples can cause merge conflicts in the future
...
Each release will be denoted by 3 numbers: a.b.c meaning:
    - c: bug fixed and improved comments and documentation. Some new capabilities could be added (see changelog). The higer the number, the better.
    - b: Significant changes, versions a.b1.c1 and a.b2.c2, should run perfectly with the same inputs.  But some internal routines may have changed, so if you have your own scripts using them 'outisde the main loop' something can go wrong for you. The higher b, the more extra capabilities you have
...
- dev-branch: developers branch, may have some small bugs or not fully developed features. Include the latest features, not recomended for general public
...
- PEP 8 guide is recommended, if some piece of code is merged without this standard, the maintainers could modify your code to addapt it to this standard
