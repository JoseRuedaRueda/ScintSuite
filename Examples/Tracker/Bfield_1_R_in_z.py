...
# --- grid:
R = np.linspace(0.01, 1.0, 180).astype(np.float64)
z = np.linspace(-1.0, 1.0, 400).astype(np.float64)
# --- custom nml and paths
directory = '/afs/ipp/home/r/ruejo/ihibpsim/jrr'
files = {
    'Bfield': os.path.join(directory, 'B2.bin'),
    'Rho': os.path.join(directory, 'rho2.bin'),
    'Orbits': os.path.join(directory, 'orb2.bin'),
    'Deposition': os.path.join(directory, 'deposition2.bin'),
    'Output': os.path.join(directory, 'output2.bin'),
    'Namelist': os.path.join(directory, 'nml2.cfg')
}
nml = {
    'FIELD_FILES': {
